...
var hasEmptyChildren = function hasEmptyChildren(tree, path) {
    // TODO: Works on the first level now, need to consider path.
    return (tree.getIn(['children1']) || []).some(function (item) {
        return item.getIn(['properties', 'field']) === null;
    });
};
...
var _validateValue = function _validateValue(config, field, operator, value, valueType, valueSrc) {
    ...
    if (v != null) {
        ...
        if (typeof fn == 'function') {
            ...
            //field,
            fieldConfig];
            if (vSrc == 'field') args.push(rightFieldDefinition);
...
function addItemWhenNotEmpty(state, path, config) {
    var parentPath = path ? path.slice(0, -1) : null;
...
var setValue = function setValue(state, path, delta, value, valueType, config) {
    ...
    var calculatedValueType = valueType || (valueSrc === 'field' && value ? (0, _configUtils.getFieldConfig)(value, config).type : valueType);
    var isValid = _validateValue(config, field, operator, value, calculatedValueType, valueSrc);
